...
const AdminEmployees = () => {
  ...
  const handleSubmit=(event)=>{
    ...
    if(!name || !email || !password || !date_of_birth || !role || !gender || !salary || !contact || !address){
      ...
      alert("please enter contact with minimum 10 digist");
    }
   else{
        ...
        <table>
         ...
         {empData.map((emp)=>(
            <div className="news-card" key={emp.id}>
                <div className='news-content'>
                  <div style={{display:"flex",columnGap:"10px",justifyContent:"space-evenly"}}>
                   ...
                   {/* <div className="news-image">
                    ...
                    <div>
                    <h4 className='news-description'>Id:{emp.emp_id}, Name:{emp.name}</h4>
                    <h4 className='news-description'>Email:{emp.email}</h4>
                    <h4 className='news-description'>Role:{emp.role} </h4>
                    <h4 className='news-description'>Status:{emp.workingStatus}</h4>
                    </div>
                    </div>
                </div>
                ...
                <button className="btn btn-danger" onClick={()=>handleDelete(emp.emp_id)}>
